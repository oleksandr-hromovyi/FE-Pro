...
let form = document.querySelector(`#formJoke`),
	...
	formSubmit = () => {
	    ...
	    if (category === 'random') {
	        ...
	        this.getJoke (`${DOMAIN}/jokes/search?query=${searchInput.value}`)
	...
	getJoke = url => {
        ...
        xhr.addEventListener(`readystatechange`, () => {
            ...
            if (jokeData.result && jokeData.result.length){
                jokeData.result.forEach(jokeData => this.renderJoke(jokeData, true, jokeContainer));
            } else if (!jokeData.result) {
                this.renderJoke(jokeData, false, jokeContainer);
            }
            }
        })
    }

    renderJoke = (jokeData, multiply=false, container, checked=false) => {
    ...
    let localJokes = this.getLocalJokes();
    localJokes.forEach(jokeData => this.renderJoke(jokeData, false, jokeContainerFav, true))
    }
